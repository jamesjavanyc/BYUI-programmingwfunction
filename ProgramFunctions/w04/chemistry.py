...
# @Author  : Junzhou Liu
# @FileName: chemistry.py.py
# @Software: PyCharm
# save the original data into txt file and trans
def data_modifier():
    with open('data.txt', 'r') as source, open('target.txt', 'w') as target:
        for line in source:
            data_list = line.split(',')
            data = data_list[0].strip() + ': [' + data_list[1].strip() + ', ' + data_list[2].strip() + '],\n'
            target.write(data)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for item in symbol_quantity_list:
        molar_mass = molar_mass + item[1] * periodic_table_dict.get(item[0])
    return molar_mass
...
def main():
    ...
    formula = input('Chemical formula:').strip()
    ...
    num_of_molar = mass / molar_mass
    # Print the molar mass.
    # exceed requirement
    print(f"molar mass of {formula} is {molar_mass}")
    if get_formula_name(formula, get_known_molecules_dict()) != "unknown compound":
        print(f'{formula}\'s name is {get_formula_name(formula, get_known_molecules_dict())}')
    # Print the number of moles.
    print(f"molar number of {mass}g {formula} is {num_of_molar}")
